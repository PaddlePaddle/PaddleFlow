	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.args.getMixedMock {
				var p1 = gomonkey.ApplyPrivateMethod(reflect.TypeOf(rts), "GetMixedLog",
					func(mixedLogRequest pfschema.MixedLogRequest) (pfschema.MixedLogResponse, error) {
						return pfschema.MixedLogResponse{}, nil
					})
				defer p1.Reset()
			}
			r := tt.args.req
