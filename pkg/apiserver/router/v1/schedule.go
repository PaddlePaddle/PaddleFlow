...
import (
	...
	"paddleflow/pkg/apiserver/controller/schedule"
...
func (sr *ScheduleRouter) createSchedule(w http.ResponseWriter, r *http.Request) {
	...
	var createScheduleInfo schedule.CreateScheduleRequest
	...
	// create schedule
	response, err := schedule.CreateSchedule(&ctx, &createScheduleInfo)
	if err != nil {
		logger.LoggerForRequest(&ctx).Errorf(
...
func (sr *ScheduleRouter) listSchedule(w http.ResponseWriter, r *http.Request) {
	...
	listScheduleResponse, err := schedule.ListSchedule(&ctx, pipelineID, marker, maxKeys, userFilter, fsFilter, scheduleFilter, nameFilter, statusFilter)
	if err != nil {
		common.RenderErrWithMessage(w, ctx.RequestID, ctx.ErrorCode, err.Error())
		return
	}
...
func (sr *ScheduleRouter) getSchedule(w http.ResponseWriter, r *http.Request) {
	...
	getScheduleResponse, err := schedule.GetSchedule(&ctx, scheduleID, marker, maxKeys, runFilter, RunStatusFilter)
	if err != nil {
		common.RenderErrWithMessage(w, ctx.RequestID, ctx.ErrorCode, err.Error())
		return
	}
...
func (sr *ScheduleRouter) stopSchedule(w http.ResponseWriter, r *http.Request) {
	...
	err := schedule.StopSchedule(&ctx, scheduleID)
	if err != nil {
