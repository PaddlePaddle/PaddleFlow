...
import (
	...
	controllerPpl "paddleflow/pkg/apiserver/controller/pipeline"
	controllerRun "paddleflow/pkg/apiserver/controller/run"
...
type GetScheduleResponse struct {
	ScheduleBrief
	ListRunResponse controllerRun.ListRunResponse `json:"runs"`
}
...
func CreateSchedule(ctx *logger.RequestContext, request *CreateScheduleRequest) (CreateScheduleResponse, error) {
	...
	// 校验用户对pplID pplDetailPk是否有权限
	hasAuth, err := controllerPpl.CheckPipelineDetailPermission(ctx.UserName, request.PipelineID, request.PipelineDetailPk)
	if err != nil {
		ctx.ErrorCode = common.InvalidArguments
		errMsg := fmt.Sprintf("create schedule failed, %s", err.Error())
		ctx.Logging().Errorf(errMsg)
	...
	schedule := models.Schedule{
		...
		PipelineDetailPk: request.PipelineDetailPk,
	...
	err = SendSingnal(pkgPipeline.OpTypeCreate, scheduleID)
	if err != nil {
		ctx.ErrorCode = common.InternalError
...
func SendSingnal(opType, scheduleID string) error {
	globalScheduler := pkgPipeline.GetGlobalScheduler()
	schduleOp, err := pkgPipeline.NewOpInfo(opType, scheduleID)
	if err != nil {
		return err
	}
...
func ListSchedule(ctx *logger.RequestContext, pipelineID, marker string, maxKeys int, userFilter, fsFilter, scheduleFilter, nameFilter, statusFilter []string) (ListScheduleResponse, error) {
	...
	// model list
	scheduleList, err := models.ListSchedule(ctx.Logging(), pipelineID, 0, pk, maxKeys, userFilter, fsFilter, scheduleFilter, nameFilter, statusFilter)
	if err != nil {
...
func GetSchedule(ctx *logger.RequestContext, scheduleID string, marker string, maxKeys int, runFilter, statusFilter []string) (GetScheduleResponse, error) {
	...
	listRunResponse, err := controllerRun.ListRun(ctx, marker, maxKeys, userFilter, fsFilter, runFilter, nameFilter, statusFilter, scheduleIDFilter)
	if err != nil {
...
func StopSchedule(ctx *logger.RequestContext, scheduleID string) error {
	...
	err = SendSingnal(pkgPipeline.OpTypeStop, scheduleID)
	if err != nil {
		ctx.ErrorCode = common.InternalError
...
func DeleteSchedule(ctx *logger.RequestContext, scheduleID string) error {
	...
	err = SendSingnal(pkgPipeline.OpTypeDelete, scheduleID)
	if err != nil {
		ctx.ErrorCode = common.InternalError
		errMsg := fmt.Sprintf("stop schedule failed in sending stop channel signal. error:%v", err)
		ctx.Logging().Errorf(errMsg)
