...
func Login(ctx *logger.RequestContext, userName string, password string, passwordEncoded bool) (*models.User, error) {
	...
	user, err := models.GetUserByName(database.DB, ctx, userName)
	if err != nil {
...
func CreateUser(ctx *logger.RequestContext, userName, password string) (*CreateUserResponse, error) {
	...
	if err := models.CreateUser(database.DB, ctx, &user); err != nil {
...
func UpdateUser(ctx *logger.RequestContext, userName, password string) error {
	...
	//check user exist
	if _, err := models.GetUserByName(database.DB, ctx, userName); err != nil {
		ctx.ErrorCode = common.UserNotExist
	...
	err = models.UpdateUser(database.DB, ctx, userName, newPassword)
	if err != nil {
...
func DeleteUser(ctx *logger.RequestContext, userName string) error {
	...
	if err := models.DeleteUser(database.DB, ctx, userName); err != nil {
		ctx.ErrorCode = common.InternalError
		ctx.Logging().Errorf("models delete user failed. error:%s", err.Error())
		return err
	}
	if err := models.DeleteGrantByUserName(database.DB, ctx, userName); err != nil {
		ctx.ErrorCode = common.InternalError
...
func ListUser(ctx *logger.RequestContext, marker string, maxKeys int) (*ListUserResponse, error) {
	...
	userList, err := models.ListUser(database.DB, ctx, pk, maxKeys)
	if err != nil {
...
func IsLastUserPk(ctx *logger.RequestContext, pk int64) bool {
	lastQueue, err := models.GetLastUser(database.DB, ctx)
	if err != nil {
