...
type CreateScheduleRequest struct {
	...
	GlobalFs          string `json:"globalFs"`          // optional
...
func CreateSchedule(ctx *logger.RequestContext, request *CreateScheduleRequest) (CreateScheduleResponse, error) {
	...
	_, err := CheckFsAndGetID(ctx.UserName, request.UserName, request.GlobalFs)
	if err != nil {
		ctx.ErrorCode = common.InvalidArguments
	...
	fsConfig := models.FsConfig{FsName: request.GlobalFs, UserName: request.UserName}
