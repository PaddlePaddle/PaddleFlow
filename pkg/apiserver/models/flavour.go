...
func (flavour *Flavour) AfterFind(*gorm.DB) error {
	...
	if flavour.ClusterName == "" && flavour.ClusterID != "" {
		...
		log.Debugf("flavour[%s] ClusterName is nil", flavour.Name)
...
func CreateFlavour(flavour *Flavour) error {
	...
	tx := database.DB.Table(tableName).Create(flavour)
	if tx.Error != nil {
...
func GetFlavour(flavourName string) (Flavour, error) {
	...
	var flavour Flavour
	tx := database.DB.Table(tableName)
...
func ListFlavour(pk int64, maxKeys int, clusterID, queryKey string) ([]Flavour, error) {
	log.Debugf("list flavour, pk: %d, maxKeys: %d", pk, maxKeys)

	var flavours []Flavour
	query := database.DB.Table(tableName).Where("flavour.pk > ?", pk).Select(flavourSelectColumn).Joins(flavourJoinCluster)
...
func GetLastFlavour() (Flavour, error) {
	...
	flavour := Flavour{}
	tx := database.DB.Table(tableName).Last(&flavour)
	if tx.Error != nil {
