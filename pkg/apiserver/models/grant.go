...
func CreateGrant(ctx *logger.RequestContext, grant *Grant) error {
	...
	tx := dbflag.DB.Table("grant").Create(grant)
	if tx.Error != nil {
...
func DeleteGrant(ctx *logger.RequestContext, userName, resourceType, resourceID string) error {
	...
	tx := dbflag.DB.Unscoped().Table("grant").Where("user_name = ? and resource_type = ? and resource_id = ?", userName, resourceType, resourceID).Delete(&Grant{})
	if tx.Error != nil {
...
func GetGrant(ctx *logger.RequestContext, userName, resourceType, resourceID string) (*Grant, error) {
	...
	var grant Grant
	tx := dbflag.DB.Table("grant").Where("user_name = ? and resource_id = ? and resource_type = ?", userName, resourceID, resourceType).First(&grant)
	if tx.Error != nil {
...
func HasAccessToResource(ctx *logger.RequestContext, resourceType string, resourceID string) bool {
	...
	var num int64
	tx := dbflag.DB.Table("grant").Where("user_name = ? and resource_type = ? and resource_id = ?",
...
func DeleteGrantByUserName(ctx *logger.RequestContext, userName string) error {
	...
	err := dbflag.DB.Unscoped().Table("grant").Where("user_name = ?", userName).Delete(&Grant{}).Error
	if err != nil {
...
func DeleteGrantByResourceID(ctx *logger.RequestContext, resourceID string) error {
	...
	err := dbflag.DB.Table("grant").Unscoped().Where("resource_id = ?", resourceID).Delete(&Grant{}).Error
	if err != nil {
...
func GetLastGrant(ctx *logger.RequestContext) (Grant, error) {
	...
	grant := Grant{}
	tx := dbflag.DB.Table("grant").Last(&grant)
	if tx.Error != nil {
