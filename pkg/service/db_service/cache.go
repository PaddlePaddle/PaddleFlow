...
func CreateRunCache(logEntry *log.Entry, cache *models.RunCache) (string, error) {
	logEntry.Debugf("begin create cache:%+v", cache)
	err := WithTransaction(database.DB, func(tx *gorm.DB) error {
		result := tx.Model(&models.RunCache{}).Create(cache)
		if result.Error != nil {
			logEntry.Errorf("create cache failed. cache:%v, error:%v", cache, result.Error)
			return result.Error
		}
		cache.ID = schema.PrefixCache + fmt.Sprintf("%06d", cache.Pk)
		logEntry.Debugf("created cache with pk[%d], cacheID[%s]", cache.Pk, cache.ID)
		// update ID by pk
		result = tx.Model(&models.RunCache{}).Where("pk = ?", cache.Pk).Update("id", cache.ID)
		if result.Error != nil {
...
func ListRunCacheByFirstFp(logEntry *log.Entry, firstFp, fsID, step, source string) ([]models.RunCache, error) {
	var cacheList []models.RunCache
	tx := database.DB.Model(&models.RunCache{}).Where(
...
func UpdateCache(logEntry *log.Entry, cacheID string, cache models.RunCache) error {
	logEntry.Debugf("begin update cache. cacheID:%s, new cache:%v", cache.ID, cache)
	tx := database.DB.Model(&models.RunCache{}).Where("id = ?", cacheID).Updates(cache)
	if tx.Error != nil {
...
func GetRunCache(logEntry *log.Entry, cacheID string) (models.RunCache, error) {
	logEntry.Debugf("begin get cache. cacheID:%s", cacheID)
	var cache models.RunCache
	tx := database.DB.Model(&models.RunCache{}).Where("id = ?", cacheID).First(&cache)
	if tx.Error != nil {
		logEntry.Errorf("get cache failed. cacheID:%s, error:%v", cacheID, tx.Error)
		return models.RunCache{}, tx.Error
	}
	return cache, nil
}
...
func DeleteRunCache(logEntry *log.Entry, cacheID string) error {
	...
	tx := database.DB.Model(&models.RunCache{}).Unscoped().Where("id = ?", cacheID).Delete(&models.RunCache{})
	if tx.Error != nil {
...
func ListRunCache(logEntry *log.Entry, pk int64, maxKeys int, userFilter, fsFilter, runFilter []string) ([]models.RunCache, error) {
	logEntry.Debugf("begin list cache")
	tx := database.DB.Model(&models.RunCache{}).Where("pk > ?", pk)
	if len(userFilter) > 0 {
...
func GetLastCacheForRun(logEntry *log.Entry, runID string) (models.RunCache, error) {
	logEntry.Debugf("get last cache for run:%s", runID)
	cache := models.RunCache{}
	tx := database.DB.Model(&models.RunCache{}).Where(&models.RunCache{RunID: runID}).Last(&cache)
	if tx.Error != nil {
...
func GetLastRunCache(logEntry *log.Entry) (models.RunCache, error) {
	logEntry.Debugf("get last runCache")
	runCache := models.RunCache{}
	tx := database.DB.Model(&models.RunCache{}).Last(&runCache)
	if tx.Error != nil {
...
func GetCacheCount(logEntry *log.Entry, runID string) (int64, error) {
	...
	var count int64
	query := database.DB.Model(&models.RunCache{})
	if runID != "" {
		query = query.Where(&models.RunCache{RunID: runID})
	}
