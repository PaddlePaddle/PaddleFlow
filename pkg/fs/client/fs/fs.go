...
func NewFileSystem(fsMeta base.FSMeta, links map[string]base.FSMeta, skipSub bool, hasCache bool,
...
func (fs *FileSystem) Open(path string) (*File, error) {
	...
	attr, ino, sysErr := fs.lookup(ctx, path, true)
...
func (fs *FileSystem) Stat(path string) (os.FileInfo, error) {
	ctx := meta.NewEmptyContext()
	attr, _, err := fs.lookup(ctx, path, true)
	if utils.IsError(err) {
		return nil, err
	}
...
func (fs *FileSystem) lookup(ctx *meta.Context, path string, canCache bool) (*meta.Attr, vfs.Ino, syscall.Errno) {
	if canCache && fs.cache != nil {
		key, inodeOk := fs.cache.GetEntryItem(path)
		if inodeOk {
			inode := key.(vfs.Ino)
	...
	entry, err := fs.vfs.Lookup(ctx, pInode, filepath.Base(path))
	if utils.IsError(err) {
		return nil, 0, err
	}
	log.Debugf("the pInode[%d], name[%s], entry[%+v]", pInode, filepath.Base(path), entry)
	if fs.cache != nil {
		fs.cache.SetEntryItem(path, entry.Ino)
		fs.cache.SetAttrItem(entry.Ino, entry.Attr)
	}
	return entry.Attr, entry.Ino, syscall.F_OK
}
