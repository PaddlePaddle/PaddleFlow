...
func TestInformationFromUrl(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:               args{url: "hdfs://192.168.1.2:9000,192.168.1.3:9000/myfs/path", properties: map[string]string{base.KeyTabData: "xxx"}},
		...
		{
			name:               "s3",
			args:               args{url: "s3://bucket/path", properties: map[string]string{base.Endpoint: "192.168.1.4"}},
		...
		{
			name:               "s3",
			args:               args{url: "s3://bucket", properties: map[string]string{base.Endpoint: "192.168.1.4"}},
			wantFileSystemType: "s3",
			wantServerAddress:  "192.168.1.4",
			wantSubPath:        "/",
		},
