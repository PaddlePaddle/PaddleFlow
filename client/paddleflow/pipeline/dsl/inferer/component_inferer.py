...
class ComponentInferer(object):
    ...
    def infer(self, env: Dict):
        ...
        """
        # delete artifact and parameter which was inferred by pre infer
        self._delete_infer_art_param()
    ...
    def _delete_infer_art_param(self):
        """ delete artifact and parameter which was generated by infering
        """
        # delete parameter
        self._delete_infer_param()

        # deleter artifact
        self._delete_infer_art()        

    def _delete_infer_param(self):
        """ delete parameter which was generated by infering
        """
        pattern = self._generate_infer_pattern("param")
        need_delete = []
        ...
        for key in need_delete:
            self._component.parameters.pop(key)

    def _delete_infer_art(self):
        """ delete parameter which was generated by infering
        """
        pattern = self._generate_infer_pattern("param")

        inputs_delete = []
        for key in self._component.inputs:
            if pattern.match(key):
                inputs_delete.append(key)
        
        for key in inputs_delete:
            self._component.inputs.pop(key)
        
        outputs_delete = []
        for key in self._component.outputs:
            if pattern.match(key):
                outputs_delete.append(key)

        for key in outputs_delete:
            self._component.outputs.pop(key)
